...
CREATE TABLE `experiment_plan` (
    ...
    `research_purpose` text NOT NULL COMMENT '研究目的',
    `model_info` varchar(500) NOT NULL COMMENT '模型信息',
...
CREATE TABLE `experiment_researcher` (
    ...
    `researcher_id` BIGINT NOT NULL COMMENT '研究员ID——关联用户ID',
    `status` TINYINT DEFAULT 1 COMMENT '状态（1：正常，0：删除）',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
     PRIMARY KEY (`id`),
     KEY `idx_experiment_id` (`experiment_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='实验研究员表';
...
CREATE TABLE `analysis_report` (
    ...
    `roc_image_Path` VARCHAR(500) COMMENT 'ROC曲线图片存储路径',
